...
resource "proxmox_virtual_environment_vm" "jenkins_vm" {
  ...
  initialization {
    ...
    ip_config {
      ipv4 {
        address = "${var.jenkins_ips[count.index]}/${var.network_range}"
        gateway = var.gateway
      }
    }
...
resource "proxmox_virtual_environment_download_file" "latest_ubuntu_22_jammy_qcow2_img" {
  ...
  file_name    = "jenkins-ubuntu-amd64.img"
}
...
resource "ansible_host" "jenkins_node" {
  name   = var.jenkins_ips[count.index]
  groups = ["jenkins"]
  ...
  variables = {
    ...
    host_name                    = proxmox_virtual_environment_vm.jenkins_vm[count.index].name 
    greetings                    = "from host!"
    some                         = "variable"
    private_ip                   = var.jenkins_ips[count.index]
  }
  count = var.jenkins_count
}
